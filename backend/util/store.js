...
module.exports = {
  ...
  upsertMomentData: (zoomUserId, data) => {
    const dbKey = `oxzamoments:${zoomUserId}`
    return setAsync(dbKey, encrypt.afterSerialization(JSON.stringify(data)))
  },

  getMomentData: async (zoomUserId) => {
    const dbKey = `oxzamoments:${zoomUserId}`
    const existingUser = await getAsync(dbKey)
    return await JSON.parse(encrypt.beforeDeserialization(existingUser))
  }
}
